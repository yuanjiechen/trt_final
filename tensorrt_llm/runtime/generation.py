...
class GenerationSession(object):
    ...
    def __init__(self,
        ...
        self.vocab_weight = torch.from_numpy(np.load("/root/workspace/trt_final/examples/MiniGPT-4/vocab.npy")).to(torch.device("cuda"),torch.half)
    ...
    def _get_next_step_shape_buffer(self,
        ...
        if self.remove_input_padding:
            if not hasattr(self, "output_datas"): input_shape = (1, batch_size * beam_width)
            else: input_shape = (1, batch_size * beam_width, self._model_config.hidden_size)
        else:
            if not hasattr(self, "output_datas"): input_shape = (batch_size * beam_width, 1)
            else: input_shape = (batch_size * beam_width, 1, self._model_config.hidden_size)
        next_step_shape = {
            'input_ids':
            input_shape,
            'input_lengths':
        ...
        next_step_buffer = {
            'input_ids': self.output_ids[step + max_input_length] if not hasattr(self, "output_datas") else self.output_datas[:, step + max_input_length, :],
            'logits': self.buffer['logits'],
            'input_lengths': input_lengths.contiguous(),
            'position_ids': position_ids.contiguous(),
            'last_token_ids': last_token_ids.contiguous(),
        ...
        for step in range(0, self.max_new_tokens):
            ...
            if step == 0:
                ...
                ctx_shape, ctx_buffer = self._get_context_shape_buffer(
                    input_data if input_data is not None else input_ids, max_input_length, step, masked_tokens,
