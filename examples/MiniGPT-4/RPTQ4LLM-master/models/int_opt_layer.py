...
class QuantOPTAttention(nn.Module):
    ...
    def forward(
        ...
        query_states = query_states #self.qkt_matmul.quant_x1(query_states)
        ...
        if past_key_value is not None:
            ...
            key_states = key_states #self.qkt_matmul.quant_x2(key_states)
            key_states = self._shape(key_states, -1, bsz)

            value_states = self.v_proj(hidden_states)
            value_states = value_states#self.pv_matmul.quant_x2(value_states)
        ...
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))#self.qkt_matmul()
        ...
        attn_weights = attn_weights #self.pv_matmul.quant_x1(attn_weights)

        attn_probs = nn.functional.dropout(
            attn_weights, p=self.dropout, training=self.training
        )

        attn_output = torch.bmm(attn_probs, value_states) #self.pv_matmul
