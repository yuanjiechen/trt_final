...
def get_llama(nsamples, seed, seqlen, model, cache_dir):
    print("get_llama")
    data_folder = Path(cache_dir).joinpath("quantize_data")
    label_folder = Path(cache_dir).joinpath("quantize_label")
    dataset = []
    
    for i, npy in enumerate(data_folder.glob("*.npy")):
        if i > nsamples: break
        arr = np.load(npy)
        tensor = torch.from_numpy(arr).cpu()
        tensor_cache = torch.zeros([1,128,4096]).cpu()
        tensor_cache[:,:tensor.size(1)] = tensor
        label_path = label_folder.joinpath(f"{npy.name}")
        label = np.load(label_path)
        label = torch.from_numpy(label).cpu()
        dataset.append([tensor_cache, label])
...
def get_loaders(
    name, nsamples=128, seed=0, seqlen=2048, model='',dataset=''
):
    if 'llama' in name:
        return get_llama(nsamples, seed, seqlen, model, dataset)
    if 'wikitext2' in name:
        return get_wikitext2(nsamples, seed, seqlen, model,dataset)
    if 'ptb' in name:
        return get_ptb(nsamples, seed, seqlen, model,dataset)
    if 'c4' in name:
        return get_c4(nsamples, seed, seqlen, model,dataset)
    if 'mix' in name:
        wiki_train,wiki_val=get_wikitext2(nsamples//3, seed, seqlen, model,dataset)
        ptb_train,ptb_val=get_ptb(nsamples//3, seed, seqlen, model,dataset)
        c4_train,c4_val=get_c4(nsamples//3, seed, seqlen, model,dataset)
