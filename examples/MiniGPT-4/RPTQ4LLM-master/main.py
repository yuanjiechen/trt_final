...
import random
import numpy as np
from models.llama import LLAMClass
import torch
import time
...
def main():
    ...
    parser.add_argument(
        "--llama_data", default="./llama_data", type=str, help="OPT model cache_dir"
    )
    parser.add_argument(
        "--calib_dataset",
        type=str,
        default="llama",
        choices=["wikitext2", "ptb", "c4", "mix", "llama"],
        help="Where to extract calibration data from.",
    )
    parser.add_argument(
        "--nsamples", type=int, default=3900, help="Number of calibration data samples."
    )
    parser.add_argument(
        "--percdamp",
        type=float,
    ...
    parser.add_argument("--reorder", type=str, default="145", help="like 12345 or 1")
    parser.add_argument(
    ...
    if "llama" in args.net:
        args.model = f"facebook/{args.net}"
        args.cache_dir = (
            f"/aidata/pengf/trt2023/MiniGPT-4/parameter/weight"
        )
        lm = LLAMClass(args)
        lm.model.eval()
    else:
